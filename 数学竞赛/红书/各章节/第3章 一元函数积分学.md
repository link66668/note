...
source:
  ...
  - "数学竞赛/原件/章节拆分OCR原始稿/第3章 一元函数积分学-OCR原始稿.md"
related:
  - "[[数学竞赛/目录|数学竞赛目录]]"
  - "[[数学竞赛/第2章 一元函数微分学]]"
  - "[[数学竞赛/第4章 常微分方程]]"
...
上一章：[[数学竞赛/第2章 一元函数微分学]] | 目录：[[数学竞赛/目录|数学竞赛目录]] | 下一章：[[数学竞赛/第4章 常微分方程]]
...
> [!answer]- 解答
> (I)  $I = \int \frac{\frac{1 - \ln x}{x^{2}}}{\left(1 - \frac{\ln x}{x}\right)^{2}} \mathrm{d}x = -\int \frac{\mathrm{d} \left(1 - \frac{\ln x}{x}\right)}{\left(1 - \frac{\ln x}{x}\right)^{2}} = \frac{1}{1 - \frac{\ln x}{x}} + C = \frac{x}{x - \ln x} + C.$
>
> (Ⅱ)
>
...
> [!answer]- 解答
> (I)  $I = \int \frac{(1 + x)e^{x}}{x e^{x}(1 + x e^{x})} \, \mathrm{d}x = \int \left( \frac{1}{x e^{x}} - \frac{1}{1 + x e^{x}} \right) \mathrm{d} \left( x e^{x} \right)$
>
...
> [!answer]- 解答
> 参见例 3.3, 应填答案:  $\frac{1}{\sqrt{2}}\arctan\frac{x^{2}-1}{\sqrt{2}x}+C$.
>
> 2. 换元积分法
>
...
> [!answer]- 解答
> 令 y = tx，与方程  $y^{2}(x - y) = x^{2}$ 联立，解得  $x = \frac{1}{t^{2}(1 - t)}$， $y = \frac{1}{t(1 - t)}$，则  $\mathrm{d}x = \frac{-2 + 3t}{t^{3}(1 - t)^{2}}\mathrm{d}t$。所以
>
...
> [!answer]- 解答
> (I) 可设  $x = \sqrt{5}\tan t, t \in \left(-\frac{\pi}{2}, \frac{\pi}{2}\right)$，则  $\mathrm{d}x = \sqrt{5}\sec^{2}t\mathrm{d}t$，
>
...
> [!answer]- 解答
> 设  $\sqrt{1+x}=\lambda\left(t+\frac{1}{t}\right)$,  $\sqrt{x}=\lambda\left(t-\frac{1}{t}\right)$，可求出  $\lambda=\frac{1}{2}$，则  $t=\sqrt{x}+\sqrt{x+1}$.
>
...
> [!answer]- 解答
> $\frac{1}{2}$ 和  $\frac{1}{3}$ 的公分母为 6，故可设  $t=\sqrt[6]{1+x}$，则  $\sqrt{1+x}=t^{3}$， $\sqrt[3]{1+x}=t^{2}$， $\mathrm{d}x=6t^{5}\mathrm{d}t$。故
>
...
> [!answer]- 解答
> 设  $t = \sqrt[3]{\frac{x+1}{x-1}}$，则  $x = 1 + \frac{2}{t^{3} - 1}$，从而
>
...
> [!answer]- 解答
> (I) 令  $t = x - \frac{1}{x}$，则  $\mathrm{d}t = \left(1 + \frac{1}{x^{2}}\right)\mathrm{d}x$，
>
...
> [!answer]- 解答
> 注意到  $\frac{\mathrm{d}}{\mathrm{d}x}(x\tan x+1)=x\sec^{2}x+\tan x$，以及  $\frac{\mathrm{d}}{\mathrm{d}x}(x\sin x+\cos x)=x\cos x$，所以
>
...
> [!note] 分析
> 被积函数是一个结构比较复杂的分式，根据其分母的特征，容易联想到微分运算公式： $\mathrm{d}\left(\frac{1}{f(x)}\right)=-\frac{f'(x)}{f^{2}(x)}\mathrm{d}x$。这里 $f(x)=x\sin x+\cos x$， $f'(x)=x\cos x$。由此得到启发，先将被积函数的分子与分母同时乘以 $\cos x$，并且令 $u(x)=\frac{x}{\cos x}$， $v'(x)=\frac{x\cos x}{(x\sin x+\cos x)^{2}}$，再利用分部积分公式即可使问题简化。
...
> [!answer]- 解答
> $I=\int\mathrm{e}^{x+\frac{1}{x}}\mathrm{d}x+\int\left(x-\frac{1}{x}\right)\mathrm{e}^{x+\frac{1}{x}}\mathrm{d}x\triangleq I_{1}+I_{2}$，其中  $I_{1}=\int\mathrm{e}^{x+\frac{1}{x}}\mathrm{d}x$，而
>
...
> [!note] 注
> 这里，利用行列式来表述较为复杂的计算式子，具有过程简捷、结构紧凑、规律性强等明显优点.
...
> [!answer]- 解答
> 令  $t = \tan \frac{x}{2}$，则  $\sin x = \frac{2t}{1 + t^{2}}$， $\cos x = \frac{1 - t^{2}}{1 + t^{2}}$， $dx = \frac{2}{1 + t^{2}}dt$。从而
>
...
> [!note] 分析
> 若采用部分分式法求解本题，则在将被积函数分解成部分分式之和的形式时，由于
...
> [!answer]- 解答
> 令  $x = \tan t$，则  $\mathrm{d}x = \sec^{2}t\mathrm{d}t$。于是
>
...
> [!note] 分析
> 这里，被积函数是三角函数有理式，可采用万能代换  $t = \tan \frac{x}{2}$ 化为有理分式函数，再利用部分分式法求解。这将是一个复杂的计算过程。因此，应考虑综合运用换元积分、分部积分等方法与技巧。

> [!answer]- 解答
> $I=\int\frac{\sec^{2}x\mathrm{d}x}{\sec^{2}x(\sin x+2\sec x)^{2}}=\int\frac{\mathrm{d}(\tan x)}{\left(2\tan^{2}x+\tan x+2\right)^{2}}\xlongequal{t=\tan x}\int\frac{\mathrm{d}t}{\left(2t^{2}+t+2\right)^{2}}$
>
...
> [!answer]- 解答
> (I) I =  $\int_{0}^{\frac{\pi}{4}}\frac{\mathrm{d}x}{1+\cos\left(\frac{\pi}{2}-x\right)}=\int_{0}^{\frac{\pi}{4}}\frac{\mathrm{d}x}{2\cos^{2}\left(\frac{\pi}{4}-\frac{x}{2}\right)}=-\left.\tan\left(\frac{\pi}{4}-\frac{x}{2}\right)\right|_{0}^{\frac{\pi}{4}}=2-\sqrt{2}.$
>
...
> [!answer]- 解答
> 这里，被积函数  $f(x)$ 是分段函数。当  $\pi \leqslant x < 2\pi$ 时， $0 \leqslant x - \pi < \pi$，因此
>
...
> [!note] 分析
> 含有绝对值的函数也是分段函数. 一般而言, 可按例 3.29 的分段积分法求解这类积分. 但在本例中, 由于积分限都是一般参数, 难以分段处理. 一种可行的方法是,

先求出被积函数的原函数, 再利用 Newton-Leibniz 公式, 但较烦琐. 下面的技巧既简捷也易掌握.

> [!answer]- 解答
> 若 a > 0, b > 0，则  $I = \int_{a}^{0} x e^{-x} \mathrm{d} x = -\left(x + 1\right) e^{-x} \bigg|_{a}^{b} = (a + 1) e^{-a} - (b + 1) e^{-b}$
>
...
> [!answer]- 解答
> 因  $x(1-x)=\frac{1}{4}-\left(\frac{1}{2}-x\right)^{2}$，故可令  $\frac{1}{2}-x=\frac{1}{2}\cos t$，则
>
...
> [!answer]- 解答
> (I) 被积函数  $\sqrt{1 - \sin 2x} = |\cos x - \sin x|$ 以  $\pi$ 为周期，故由上述公式 (4) 有
>
...
> [!answer]- 解答
> (I) 根据积分区间与被积函数的特征, 可利用公式(5)求解. 故有
>
...
> [!answer]- 解答
> 对任意 x，恒有  $\arctan e^{x} + \arctan e^{-x} = \frac{\pi}{2}$（详见本章例 3.47）。利用公式 (3)，得
>
...
> [!answer]- 解答
> 被积函数与周期函数有关, 可考虑利用公式 (4) 求解. 先作变量代换:  $u = \ln x$.
>
...
> [!answer]- 解答
> (I) 欲求  $f(2)$，需先求出  $f(x)$. 故只要将所给方程两端对 x 求导即可.
>
...
> [!answer]- 解答
> 被积函数含变上限 x. 令 u = 2x - t，则  $t = 2x - u, \mathrm{d}t = -\mathrm{d}u$，从而
>
...
> [!answer]- 解答
> 首先, 由方程  $e^{x-2y}-x^{2}y=1$ 可知, 当 x=0 时 y=0, 即  $f(0)=0$, 并根据隐函数求导法则得  $\mathrm{e}^{x-2y}(1-2y')-2xy-x^{2}y'=0$, 从而有  $f'(0)=y'(0)=\frac{1}{2}$.
>
...
> [!answer]- 解答
> 因为  $g(x)$ 在  $[a,b]$ 上连续且恒正，所以  $g(x)$ 在  $[a,b]$ 上存在最小值 m 与最大值 M，且 m > 0, M > 0。注意到  $f(x) \geqslant 0$，并利用定积分的性质，得
>
...
> [!note] 注
> 利用本题的结论，可知 $\lim_{x\to+\infty}\frac{1}{x}\int_{0}^{x}\left|\sin t\right|dt=\frac{2}{\pi}$
...
> [!note] 分析
> 对于给出函数具有二阶以上导数的条件的题目，我们往往优先考虑利用 Taylor 公式来解。但应注意到 Taylor 公式的每一项都具有 k 阶导数  $f^{(k)}(x_0)$ 与增量的 k 次幂  $(x - x_0)^k$ 对应相乘这一特征。而在本题中，欲证明的等式右端第二项是二阶导数与三次幂相乘。因此可考虑构造一个辅助函数  $F(x)$，使  $F'''(x) = f''(x)$，并对  $F(x)$ 利用 Taylor 公式。自然，最容易想到的是  $F(x) = \int_a^x f(t) \, dt$。至此，解题思路即有了端倪。
...
> [!note] 分析
> 本题有明确的几何意义，给出了定积分的近似计算中“梯形公式”的理论支持，即对于小区间  $[a,b]$ 上的连续函数  $f(x)$，用  $\frac{1}{2}[f(a)+f(b)](b-a)$ 作为定积分  $\int_{a}^{b}f(x)\mathrm{d}x$ 的近似值，其“截断误差”为  $-\frac{1}{12}f''(\xi)(b-a)^3$， $\xi\in(a,b)$.
...
> [!answer]- 解答
> 记  $I=\int_{0}^{+\infty}\frac{\ln x}{x^{2}+a^{2}}\mathrm{d}x$．作变量代换：x=at，得
>
...
> [!note] 注
> 这里，利用了已知结果，即 Gauss 积分： $\int_{0}^{+\infty}e^{-x^{2}}dx=\frac{\sqrt{\pi}}{2}$
...
> [!note] 注
> 我们证明的这个等式称为 Frullani（傅汝兰尼）积分公式，在计算广义积分时，有时可利用这个公式直接给出计算结果.
...
> [!answer]- 解答
> 设所作切线与抛物线相切于点  $(x_{0}, \sqrt{x_{0}-2})$. 因为
>
...
> [!note] 分析
> 曲线  $\left\{\begin{array}{l}x=\cos^{3}t,\quad(0\leqslant t\leqslant\frac{\pi}{2})\\y=\sin^{3}t\end{array}\right.$ 是用参数方程表示的星形线，位于第一象限的部分，其直角坐标方程为  $x^{\frac{2}{3}}+y^{\frac{2}{3}}=1(0\leqslant x\leqslant1)$. 主要注意两点：一是两条曲线的交点  $(1,0)$ 与  $(0,1)$; 二是两条曲线的位置关系，圆  $y=\sqrt{1-x^{2}}$  $(0\leqslant x\leqslant1)$ 位于星形线的上方.

> [!answer]- 解答
> 设 D 绕 x 轴旋转一周所得旋转体的体积为 V，表面积为 A，则
>
...
> [!note] 注
> 这里，在计算 $\int_{0}^{\frac{\pi}{2}}\sin^{7}tdt$ 和 $\int_{0}^{\frac{\pi}{2}}\sin^{9}tdt$ 时直接利用了 Wallis 公式.
...
> [!answer]- 解答
> 首先容易求得两曲线的交点为  $(8a, \pm 4\sqrt{2}a)$，而渐屈线交 x 轴于点  $(2a, 0)$（图 3.2）。由于渐屈线在上半平面的方程为
>
...
> [!answer]- 解答
> 这里，所给曲线的方程是极坐标表达式。只要能确定 $\theta$的变化范围，即可利用相应的公式计算出所求弧长。
>
...
> [!note] 注
> 长度单位：m，重力加速度为  $g \, m/s^{2}$，水的密度为  $1 \times 10^{3} kg/m^{3}$.

> [!answer]- 解答
> (I) 由对称性知，容器位于  $y = \frac{1}{2}$ 上、下两侧部分的容积相等，因此，只需考察  $-1 \leqslant y \leqslant \frac{1}{2}$ 部分，曲线可表示为
>
...
> [!note] 分析
> 质量分别为  $m_{1}, m_{2}$ 相距为 r 的两个质点间的引力大小为
...
> [!answer]- 解答
> (I) 以 O 为原点、 $\overrightarrow{OA}$ 方向为 x 轴建立坐标系。在 x 轴上取小区间  $[x, x + \mathrm{d}x]$，对应细杆的一小段，其质量为  $\frac{M}{l} \mathrm{d}x$，与质点 B 间的引力大小为
>
...
> [!answer]- 解答
> 如图 3.5 所示建立坐标系，则抛物线的方程为 y =  $x^{2}$. 闸门的矩形部分承受的水压力
>
...
> [!note] 分析
> 若设  $f(a)=\int_{0}^{a}\left(1-x^{2}\right)^{\frac{5}{2}}\mathrm{d}x-\frac{5a\pi}{32}$，则所证不等式即  $f(a)\geqslant0(0\leqslant a\leqslant1)$，换言之，即不存在  $\xi\in[0,1]$，使得  $f(\xi)<0$。我们采用反证法证明之。
...
> [!note] 注
> 本例综合考察连续函数的性质、凸曲线的特征、Lagrange中值定理以及定积分的性质等知识点，并且还需确定二次函数 $x(1-x)$在区间 $[0,1]$上的最大值.
...
> [!note] 注
> 本例即积分形式的 Jensen（詹森）不等式，取  $f(x)=\ln x, a=1$，则是第十届（2018年）全国初赛题（详见例3.90）：设  $u(x)$ 在  $[0,1]$ 上连续，且  $u(x)>0$，则
...
> [!note] 注
> 特别地，考虑 m = 1, M = 3，即第十届全国初赛题（2018年），详见本章

> [!note] 注
> 特别地，考虑 m=1,M=3，即第十届全国初赛题（2018年），详见本章例3.97.
...
> [!note] 分析
> 从几何上看，本题所设条件是：连续曲线  $y = f(x)$ 是向下凸的，如图 3.6，因而右边的不等式比较容易证明．而对于左边的不等式，即矩形  $aCDb$ 的面积不超过曲边梯形  $aAQBb$ 的面积，则需将定积分  $\int_{a}^{b} f(x) \, dx$ 分成两部分  $\int_{a}^{\frac{a+b}{2}} f(x) \, dx$ 与  $\int_{\frac{a+b}{2}}^{b} f(x) \, dx$，并设法“叠加”成一项，以体现几何上的“割补”措施.
...
> [!note] 注
> 本题也可构造辅助函数  $f(x)=bx-\mathrm{e}^{x-1}-b\ln b$ (b>1)，证明  $f(\ln b+1)=0$ 是  $f(x)$ 在  $(1,+\infty)$ 上的最大值，故当 a>1 时， $f(a)\leq0$，即  $ab\leq\mathrm{e}^{a-1}+b\ln b$.
...
> [!note] 注
> Cauchy 积分不等式在数学竞赛中往往被用来证明其他积分不等式.
...
> [!note] 分析
> 注意到  $\sin 2x = 2 \sin x \cos x$，作变量代换： $t = \sin x$，原积分化为  $2 \int \frac{t e^{-t}}{(1-t)^2} dt$，再利用分部积分法求解.

> [!answer]- 解答
> 原式  $= 2 \int \frac{\mathrm{e}^{-\sin x} \sin x}{(1 - \sin x)^2} \mathrm{d}(\sin x) \xlongequal{\text{令 } t = \sin x} 2 \int \frac{t \mathrm{e}^{-t}}{(1 - t)^2} \mathrm{d}t.$ 下面用两种方法计算.
>
> （方法1）
>
...
> [!answer]- 解答
> 令  $S = \sum_{n=1}^{100} n^{-\frac{1}{2}}$，问题即求不超过 S 的最大整数 [S]。因为
>
...
> [!note] 分析
> 注意到  $\left|\sin x\right|$ 是周期为  $\pi$ 的周期函数，所以应考虑先计算有限个区间上的定积分，再利用广义积分的定义求解.

> [!answer]- 解答
> 对任意 t > 0，必存在正整数 n，使得  $n\pi < t \leq (n+1)\pi$，从而有
>
...
> [!answer]- 解答
> 这样的函数不存在. 假设存在这样的函数  $f(x)$, 则由  $\left|f'(x)\right| \leqslant 1$, 得 -1  $\leqslant f'(x) \leqslant 1$. 对  $f(x)$ 分别在区间  $[0,1]$ 与  $[1,2]$ 上应用 Lagrange 中值定理, 并利用  $f(0) = f(2) = 1$, 得
>
...
> [!note] 注
> 结合上述几何直观上分析，类似地可证：设函数  $f(x)$ 在  $[0,2]$ 上连续，在  $(0,2)$ 内可导， $f(0)=f(2)=1$，且  $\left|f'(x)\right|\leqslant1$。求证： $1<\int_{0}^{2}f(x)dx<3$。
...
> [!answer]- 解答
> (1) 斜渐近线的方程为 y = kx + b，其中  $k = \lim_{x \to \infty} \frac{y}{x}$， $b = \lim_{x \to \infty} (y - kx)$.
>
...
> [!note] 分析
> 注意到 $\int_{0}^{1}f(x)\mathrm{d}x=-\int_{0}^{1}xf^{\prime}(x)\mathrm{d}x$，所以可根据题设等式“凑”一个简单的函数 $g(x)$，使得 $\int_{0}^{1}\left[f^{\prime}(x)-g(x)\right]^{2}\mathrm{d}x=0$，从而有 $f^{\prime}(x)=g(x)$。再积分即可得 $f(x)$。

> [!answer]- 解答
> 因为  $f'(x)$ 在  $[0,1]$ 上连续，所以  $\int_{0}^{1}f'(x)\mathrm{d}x=f(1)-f(0)=0.$ 又由于
>
...
> [!answer]- 解答
> 根据题设条件及积分中值定理的几何意义，可推断出  $\lim_{n\to\infty}x_n=b$，下证之.
>
...
> [!note] 注
> 这是 Kantorovich 不等式的特别情形 (m=1, M=3)，详见本章例 3.73.
...
> [!note] 分析
> 先利用待定系数法，确定一个函数  $g(x)=x^{2}+ax+b$，使得  $g(x)$ 在  $[0,1]$ 上不变号，且  $\int_{0}^{1}g(x)\left[f'(x)-3\right]\mathrm{d}x=0$，再根据积分中值定理即可证得结论.
...
> [!answer]- 解答
> 令  $f(x)=\frac{x}{\cos^{2}x+x^{\alpha}\sin^{2}x}$，因为  $f(x)$ 在  $[0,+\infty)$ 上连续，所以定积分  $\int_{0}^{1}f(x)\mathrm{d}x$ 存在，故只需讨论  $\int_{1}^{+\infty}f(x)\mathrm{d}x$ 的敛散性.
>
...
> [!answer]- 解答
> 因为  $(1-x)^{4}=(1+x^{2}-2x)^{2}=(1+x^{2})^{2}-4x(1+x^{2})+4x^{2}$，所以
>
...
> [!note] 注
> 细心的读者可能已经发现，本题的计算结果由两个重要常数构成：圆周率  $\pi$ 与圆周率的约率  $\frac{22}{7}$，这表明用  $\frac{22}{7}$ 作为  $\pi$ 的近似值其误差可用一个定积分的值表示出来.

**例 3.106**：(第十一届全国初赛题, 2019) 定积分  $\int_{0}^{\frac{\pi}{2}}\frac{\mathrm{e}^{x}(1+\sin x)}{1+\cos x}\mathrm{d}x=$ ___.

> [!note] 分析
> 被积函数同时含有指数函数与三角函数，可考虑用分部积分法，并注意形成“回归”现象.
...
> [!note] 分析
> 被积函数含有取整函数，往往需考虑分段积分，每一小段的长度为1，再分析在每一个小区间上积分的特征.
...
> [!note] 注
> 不等式具有明显的几何意义：光滑曲线  $y = f(x)$ 弧上点  $A(a, f(a))$ 与  $B(b, f(b))$ 之间的弧长不小于直线段 AB 之长.
...
(15) $\frac{1}{2}\tan^{2}x+\ln|\tan x|+C$; (16) $\sqrt{x(x+1)}+\ln(\sqrt{x}+\sqrt{1+x})-2\sqrt{x}+C$;
 ...
 $$(19)\;\frac{x}{x+\cos x}+C;\quad(20)\;-\frac{\sqrt{1-x^{2}}}{x}\arcsin x+\ln\left|x\right|+\frac{1}{2}(\arcsin x)^{2}+C.$$ 
...
> [!note] 注
> 本题的物理意义：两根长度同为1的细直杆展布在x轴的区间[0,1]上，它们的线密度函数分别为 $f^{2}(x)$与 $f(x)$，重心坐标分别为 $x_{1},x_{2}$，则由本题结论知 $x_{1}\leq x_{2}$.
