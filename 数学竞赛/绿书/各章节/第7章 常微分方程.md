...
source:
  ...
  - "数学竞赛/绿书/原件/章节拆分OCR原始稿/第7章 常微分方程-OCR原始稿.md"
related:
  - "[[数学竞赛/绿书/各章节/目录|绿书目录]]"
...
上一章：[[数学竞赛/绿书/各章节/第6章 多元向量值函数积分学|第6章 多元向量值函数积分学]]  |  下一章：[[数学竞赛/绿书/各章节/第8章 无穷级数|第8章 无穷级数]]
...
**例 1**：设  $f(x)$ 在  $(0,+\infty)$ 内可导， $f(x)>0$，且  $\lim_{x\to+\infty}f(x)=1$，又  $\lim_{h\to0}\left[\frac{f(x+xh)}{f(x)}\right]^{\frac{1}{h}}=e^{\frac{1}{x}}$，求  $f(x)$.

> [!note] 分析
> 利用导数的定义算出等式左边的极限就可得到关于  $f(x)$ 的微分方程.
...
> [!note] 分析
> 由于 f 处处可微，且所给等式关于变量 x, y 对称，所以在等式两边分别对 x, y 求偏导数，就容易消掉复合函数部分的导数，从而得到形式较简单的微分方程.

> [!answer]- 解答
> 方法1 在所给等式两边分别对 x 和 y 求偏导数，得
>
...
> [!note] 分析
> 题设没有函数可导的信息，无法直接建立微分方程。但可以利用 f 连续来做积分，得到变限积分再求导，从而建立微分方程。

> [!answer]- 解答
> 记  $\int_{0}^{1}f(x)dx=a$ ，对任意固定的 x ，等式两边对 y 在  $[0,1]$ 上积分，有
>
...
**例 4**：求方程 $x\mathrm{d}y-y\mathrm{d}x=\sqrt{x^{2}+y^{2}}\mathrm{d}x$的通解.

> [!note] 分析
> 这是齐次方程，作变量代换  $u=\frac{y}{x}$，方程化为变量可分离方程.

> [!answer]- 解答
> $\frac{dy}{dx}=\frac{y+\sqrt{x^{2}+y^{2}}}{x}=\frac{y}{x}\pm\sqrt{1+\left(\frac{y}{x}\right)^{2}}$ （x>0，根式取正；x<0，根式取负）.
>
...
**例 5**：求微分方程  $2yy' = e^{\frac{x^{2} + y^{2}}{x}} + \frac{x^{2} + y^{2}}{x} - 2x$ 的通解.

> [!note] 分析
> 难以直接看出方程的类型，凑微分方程化为 $(x^2 + y^2)' = e^{\frac{x^2 + y^2}{x}} + \frac{x^2 + y^2}{x}$，令 $x^2 + y^2 = u$，易知是齐次方程.

> [!answer]- 解答
> 令  $x^{2}+y^{2}=u$ ，则  $2x+2yy^{\prime}=u^{\prime}$ ，方程化为
>
...
**例 6**：求解微分方程  $(x-2\sin y+3)\mathrm{d}x-(2x-4\sin y-3)\cos y\mathrm{d}y=0$.

> [!note] 分析
> 该方程有多种解法，作变量代换可化为变量可分离的方程；该方程也是全微分方程.
...
> [!note] 分析
> 利用所给函数关系式来建立微分方程. 由于题设只给出了函数在点 x=0 可导的信息, 所以
>
> 只能利用导数的定义来求函数的导数.

> [!answer]- 解答
> 令 x = y = 0，得  $f(0) = f(0) + f(0) \Rightarrow f(0) = 0$。则
>
...
**例 8**：求微分方程  $y' = \frac{\cos y}{\cos y \sin 2y - x \sin y}$ 的通解.

> [!note] 分析
> 若把 x 作为自变量，则这不属于我们熟悉的基本方程类型。由于方程中的 x 是一次的，若把 y 作为自变量，x 作为未知函数，则它就是一阶线性微分方程。

> [!answer]- 解答
> $\frac{dx}{dy}=\frac{\cos y\sin 2y-x\sin y}{\cos y}=\sin 2y-x\tan y$，方程为
>
...
> [!note] 分析
> 积分方程，两边求导化为微分方程. 同时要注意利用方程确定定解条件.

> [!answer]- 解答
> 由  $f(x)$ 连续知右端函数可导，方程两边求导得
>
...
> [!note] 分析
> 一阶线性方程容易求得其解. 为便于讨论极限，解的公式中用定积分表示较好.

> [!answer]- 解答
> 初值问题写成  $\left\{\begin{aligned}&\frac{\mathrm{d}y}{\mathrm{d}x}-\left(2x+\frac{1}{x}\right)y=x,&x\geqslant1\\ &y(1)=y_{1}\end{aligned}\right.$
>
> 由一阶线性方程的求解公式得到
>
...
>
>  $$y(x)=\mathrm{e}^{-\int_{x_{0}}^{x}p(t)\mathrm{d}t}\left(\int_{x_{0}}^{x}q(t)\mathrm{e}^{\int_{x_{0}}^{t}p(s)\mathrm{d}s}\,\mathrm{d}t+y_{0}\right).$$
>
> 若  $y_{0}=y(x_{0})$，上式就是满足该初值的特解；若  $y_{0}$ 是任意常数，上式就是通解.
>
> 本题方程的解若用不定积分表示，就很难讨论其极限情况. 下题也类似.

**例 11**：设  $f(x)$ 在  $(-\infty,+\infty)$ 连续且有界，又设  $\int_{-\infty}^{0}e^{x}f(x)dx$ 收敛. 求证：
...
> [!note] 分析
> 这是一阶线性常系数微分方程的特解问题. 只需求出其解再验证条件.

> [!answer]- 证明
> （1）方法1 设  $y(x)$ 是所给方程的有界解. 方程两边乘以积分因子  $e^{\int dx} = e^{x}$，得
>
...
> [!note] 分析
> $Pdx + Qdy = 0$ 是全微分方程的充要条件是  $\frac{\partial Q}{\partial x} = \frac{\partial P}{\partial y}$，由此可求得  $f(x)$.

> [!answer]- 解答
> 记  $P(x,y)=(6y^{4}+x^{2}y^{5})f(x)$， $Q(x,y)=(8xy^{3}+x^{3}y^{4})f(x)$，由  $\frac{\partial Q}{\partial x}=\frac{\partial P}{\partial y}$，得
>
...
> [!note] 分析
> 两个方程都难以判断其类型，看能否用上题评注中的方法找到积分因子，或通过凑微分化为全微分方程.
>
...
> [!note] 分析
> 若曲面积分与曲面的形状无关，则任意闭曲面上的积分为 0，利用高斯公式得到三重积分的恒等式，可得到对应的微分方程.

> [!answer]- 解答
> 以 L 为边界任作两个光滑曲面  $S_1$、 $S_2$，它们的法向量指向同一侧，由题意有  $\iint_{S_1} = \iint_{S_2}$ 。记  $S^*$ 为  $S_1$ 与  $S_2$ 所围成的闭曲面，取外侧，则  $\iint_{S^*} = \iint_{S_1} + \iint_{S_2} = \iint_{S_1} - \iint_{S_2} = 0$。
>
...
> [!note] 分析
> 对  $S(x)$ 求导，找出  $S'(x)$ 与  $S(x)$ 的关系，也就是找到  $S(x)$ 所满足的微分方程，解方程可得  $S(x)$ 的表达式.

> [!answer]- 解答
> （1） $S(x)=\frac{x^{4}}{2\cdot4}+\frac{x^{6}}{2\cdot4\cdot6}+\frac{x^{8}}{2\cdot4\cdot6\cdot8}+\cdots$，易见  $S(0)=0$，
>
...
> [!note] 分析
> 方程的特点是不显含变量 $x$。如果令 $y' = \tan t$，代入方程可得 $y = \ln |\sec t| - \tan t$，若能求得 $x$ 关于变量 $t$ 的表达式，则微分方程的解就找到了（解为参数式函数）。
...
> [!note] 分析
> 很难看出该方程属于哪一类可求解的方程. 若能将等式右端的函数分离变量, 方程的求解就容易了. 为此, 可作变量代换  $y = x^{2} u$.

> [!answer]- 解答
> 令  $y = x^{2} u$，则  $y' = 2 x u + x^{2} u'$，代入方程化为
>
...
> [!note] 分析
> 需找出非零函数 $f(g)$和 $g$，使得 $\left(\frac{f}{g}\right)^{\prime}=\frac{f^{\prime}}{g^{\prime}}$。由于 $f$与 $g$不可交换，即 $\left(\frac{f}{g}\right)^{\prime}=\frac{f^{\prime}}{g^{\prime}}$成立时，未必有 $\left(\frac{g}{f}\right)^{\prime}=\frac{g^{\prime}}{f^{\prime}}$也成立，所以需分别讨论已知 $f$与已知 $g$的情况。

> [!answer]- 解答
> 设  $f$ 和  $g$ 是两个满足题设条件的函数，由题意  $\frac{fg - fg'}{g^2} = \frac{f'}{g'}$ 变形为
>
...
> [!note] 分析
> 显然  $y(x)$ 为严格单调递增函数，只需证明  $y(x)$ 有界即可.

> [!answer]- 证明
> 在  $y = y(x)$ 的定义域内取  $x_{0}$，记  $y_{0} = y(x_{0})$ 。将  $y = y(x)$ 代入所给方程，有
>
...
> [!answer]- 解答
> （1）记  $w(x)=v(x)-u(x)$，  $\varphi(x)=h_{2}(x)-h_{1}(x)$，由题意知，当  $x\geqslant x_{0}$ 时，  $\varphi(x)\geqslant0$
>
...
**例 1**：已知  $z = x f\left(\frac{y}{x}\right) + 2 y f\left(\frac{x}{y}\right)$，其中 f 二次可微. 若  $\left.\frac{\partial^{2}z}{\partial x\partial y}\right|_{x=a} = -by^{2}$，求函数 f.

> [!note] 分析
> f 是一元函数，对 z 的函数式求  $\frac{\sigma^{-}z}{\partial x\partial y}$，代入所给偏导数方程便可得到关于 f 的微分方程.

> [!answer]- 解答
> $\frac{\partial z}{\partial x}=f\left(\frac{y}{x}\right)-\frac{y}{x}f^{\prime}\left(\frac{y}{x}\right)+2f^{\prime}\left(\frac{x}{y}\right)$， $\frac{\partial^{2}z}{\partial x\partial y}=-\frac{y}{x^{2}}f^{\prime\prime}\left(\frac{y}{x}\right)-\frac{2x}{y^{2}}f^{\prime\prime}\left(\frac{x}{y}\right)$.
>
...
**例 2**：求微分方程  $(1-x)y''=\frac{1}{5}\sqrt{1+y'^2}$ 满足  $y(0)=0,\ y'(0)=0$ 的特解.

> [!note] 分析
> 此方程不显含 y，属于  $y'' = f(x, y')$ 型，令  $y' = p(x)$，将方程转化为一阶方程求解.
...
**例 3**：求方程 $y''+(y')^{2}=2e^{-y}$的通解.

> [!note] 分析
> 此方程不显含 x，属于  $y'' = f(y, y')$ 型，令  $y' = p(y)$，将方程转化为一阶方程求解.

> [!answer]- 解答
> 令  $y'=p(y)$，则  $y''=p\frac{dp}{dy}$，原方程可化为
>
...
**例 5**：设函数  $y = f(x)$ 由参数方程  $\begin{cases} x = 2t + t^2 \\ y = \psi(t) \end{cases}$ ( $t > -1$) 所确定，且  $\frac{d^2 y}{dx^2} = \frac{1}{4(1 + t)}$，其中  $\psi(t)$ 具有二阶导数，曲线  $y = \psi(t)$ 与曲线  $y + e^{-\frac{1}{2}y} = t$ 在  $t = 1$ 处相切，求函数  $\psi(t)$.

> [!note] 分析
> 由参数方程求得 $\frac{d^2y}{dx^2}$，代入等式 $\frac{d^2y}{dx^2}=\frac{1}{4(1+t)}$可得到 $\psi(t)$的二阶方程，其余条件即为方程的定解条件.
...
> [!note] 分析
> 积分方程，求导去掉积分号化为微分方程求解. 为达到去掉积分号的效果，求导前需对方程适当变形.
...
> [!note] 分析
> 这是关于未知函数 y 及其导数的二次齐次方程，作变量代换  $y = e^{\int z \, dx}$，可将所求方程化为未知函数 z 的一阶方程.

> [!answer]- 解答
> 设  $y = e^{\int z \, dx}$，则  $y' = z e^{\int z \, dx}$， $y'' = (z' + z^2) e^{\int z \, dx}$，代入原方程，化简得
>
...
**例 8**：设当x>-1时，可微函数 $f(x)$满足条件 $f'(x)+f(x)-\frac{1}{x+1}\int_{0}^{x}f(t)dt=0$，且 $f(0)=1$，证明当 $x\geq0$时，有 $e^{-x}\leq f(x)\leq1$成立.

> [!note] 分析
> 方程中含有未知函数的积分形式，需求导消去积分化为一个二阶微分方程. 求解微分方程，再对  $f(x)$ 做放缩来得到所证结论.

> [!answer]- 解答
> 将所给等式变形为  $\left[f'(x) + f(x)\right](x + 1) = \int_{0}^{x} f(t) \, dt$. 两边对 x 求导得
>
>  $$\begin{aligned}\left[f^{\prime\prime}(x)+f^{\prime}(x)\right](x+1)+\left[f^{\prime}(x)+f(x)\right]&=f(x),\\ (x+1)f^{\prime\prime}(x)+(x+2)f^{\prime}(x)&=0.\end{aligned}$$
>
> 即有
>
...
| 特征根 | 通解 |
|---|---|
| $r_1 \ne r_2$ 为实根 | $y = C_1e^{r_1x} + C_2e^{r_2x}$ |
| $r_1 = r_2$ 为重实根 | $y = (C_1 + C_2x)e^{r_1x}$ |
...
| $f(x)$ 的类型 | 特征根 | 特解形式 |
|---|---|---|
| $e^{\lambda x}P_m(x)$，其中 $P_m(x)$ 为 $m$ 次多项式 | $\lambda$ 是 $k$ 重特征根（$k=0,1,2$） | $y^{*}=x^{k}e^{\lambda x}Q_m(x)$，其中 $Q_m(x)$ 为 $m$ 次系数待定多项式 |
| $e^{\lambda x}[P_l(x)\cos\omega x+P_n(x)\sin\omega x]$，其中 $P_l(x),P_n(x)$ 分别为 $l,n$ 次多项式 | $\lambda\pm i\omega$ 是 $k$ 重特征根（$k=0,1,2$） | $y^{*}=x^{k}e^{\lambda x}[R_m^{(1)}(x)\cos\omega x+R_m^{(2)}(x)\sin\omega x]$，其中 $R_m^{(1)}(x),R_m^{(2)}(x)$ 为 $m$ 次系数待定多项式，$m=\max\{l,n\}$ |
...
**例 9**：求$y'' + 2ay' + a^2y = e^{bx}$的通解，a > 0，a，b 为常数.

> [!note] 分析
> 常系数非齐次线性微分方程的解法是：由特征方程法写出对应齐次方程的通解，由待定系数法确定非齐次方程的特解，再由非齐次方程解的结构写出其通解.

> [!answer]- 解答
> 对应齐次方程的特征方程为$r^{2}+2ar+a^{2}=0$，特征根为二重根$r_{1,2}=-a$，齐次方程通解为
>
...
> [!note] 分析
> 方程右端的自由项是两个不同类型的函数之和，要利用解的叠加原理分别求特解，两特解之和就是原方程的特解.

> [!answer]- 解答
> 特征方程为$r^{2}-3r+2=0$，两个特征根分别为$r_{1}=2$，$r_{2}=1$。对应的齐次方程通解为$Y=C_{1}e^{x}+C_{2}e^{2x}$。
>
...
**例 11**：求$y'' + 2y' - 3y = f(x)$满足$y(0) = 0$,$y'(0) = 1$的特解，其中$f(x) = \left\{ \begin{aligned} & x + 1, & x \geq 0 \\ & e^{x}, & x < 0 \end{aligned} \right.$

> [!note] 分析
> 由于自由项$f(x)$是一个分段函数，所以求解方程时要分段进行.
...
**例 12**：设$f(x)$在$(-∞,+∞)$二阶可导，且对任意x,y有$f^{2}(x)-f^{2}(y)=f(x+y)f(x-y)$，求$f(x)$.

> [!note] 分析
> 要建立微分方程，在方程两边需对 x 或 y 求偏导. 若求一阶导数不能化简方程，则可求二阶.

> [!answer]- 解答
> 取 x=y=0，可得$f(0)=0$，方程两边对 x 求偏导得
>
...
> [!note] 分析
> 由题设条件容易得到对应齐次方程的两个线性无关的解，从而可写出对应齐次线性微分方程。再由非齐次线性微分方程的一个特解就可确定自由项；也可根据非齐次线性微分方程解的结构写出通解，再消去通解中的任意常数得到微分方程。

> [!answer]- 解答
> 方法1 因为$y_{1}, y_{2}, y_{3}$是非齐次线性微分方程的解，则$y_{3} - y_{1} = -e^{-x}$及$y_{3} - y_{2} = e^{2x}$是对应齐次方程的解，则 r = -1, r = 2 是齐次方程的特征根，特征方程为
>
...
> [!note] 分析
> 对所给线性微分方程组消元，化为一个未知函数的二阶线性微分方程求解.
...
> [!note] 分析
> 这是二阶常系数非齐次线性微分方程，但自由项不属于用待定系数法的类型。由于对应齐次微分方程的通解易于求得，因此用常数变异法去求非齐次方程的特解。
...
**例 16**：设$f(x)$具有二阶导数，且$f(x) + f'(\pi - x) = \sin x$，$f\left(\frac{\pi}{2}\right) = 0$。求$f(x)$。

> [!note] 分析
> 方程中含$f(x)$与$f'(\pi - x)$，其中变元分别为 x 与$\pi - x$，没法求解。为使方程中只保留一种变元形式，可对方程两边求导得到新的方程，再联立消元、求解。

> [!answer]- 解答
> 由$f(x) + f'(\pi - x) = \sin x$，两边对 x 求导，有
>
...
> [!note] 分析
> 由特征根可确定方程中的系数 p 与 q. 为得到积分形式的解，需要作变量代换将二阶微分方程降为一阶方程，再积分求解.

> [!answer]- 证明
> 因$r_{1}+r_{2}=-p$，$r_{1}\cdot r_{2}=q$，所以原方程可化为
>
...
> [!note] 分析
> 我们已经知道一阶线性微分方程的比较定理（7.1 节例 20），故只需作变量代换将二阶线性方程转化为一阶线性方程来处理.

> [!answer]- 证明
> 因为$p^{2}-4q\geqslant0$，所以对应齐次方程的特征根为实数，设为 a,b，则有$p=-(a+b)$，q=ab。从而
>
...
**例 19**：求$y'' - \frac{3}{x-1} y' - \frac{5}{(x-1)^2} y = 3$的通解.

> [!note] 分析
> 方程两边同时乘以$(x-1)^2$，是欧拉方程。将$x-1=e^t$转化为常系数非齐次线性方程。

> [!answer]- 解答
> 方程为$(x-1)^{2}y''-3(x-1)y'-5y=3(x-1)^{2}$，这是一个欧拉方程.
>
...
> [!note] 分析
> 只需对函数$u(r)$求导（一至二阶），将方程$f_{11}+f_{22}=\frac{1}{r}$转化为$u(r)$的常微分方程求解.
...
> [!note] 分析
> 这是二阶线性齐次方程，只需求得两个线性无关的的特解就可得到通解. 由方程的系数$(\cos x - \sin x) - 2\cos x + (\cos x + \sin x) = 0$，知$y = e^{-x}$是方程的一个解，方程的另一个解可通过常数变异法求得.

> [!answer]- 解答
> 易验证$y = e^{-x}$是所给方程的一个解. 设$y = ue^{-x}$是方程的另一个解, 则
>
...
> [!note] 分析
> 这是二阶齐次线性微分方程. 要求方程的通解，首先要确定方程中的未知函数$q(x)$，再分别求得方程的两个线性无关的特解即可. 显然$q(x)$可由方程的任意一个非零解唯一确定.

> [!answer]- 解答
> 因为$y_{1}y_{2}=1$，若$y_{1},y_{2}$中有一个为常函数，则另一个也必为常函数，且均非 0。设$y_{1}=a\neq0$，代入微分方程，可得$q(x)=0$，此时微分方程为
>
...
> [!note] 分析
> 这是二阶非齐次线性微分方程，变系数，又不是欧拉方程，不便于求解. 看是否能通过凑微分化为“恰当方程”来降阶.
...
> [!note] 分析
> 方程是二阶变系数线性微分方程，不易直接求解。注意到$\frac{1}{\cos^{2}x}dx=d\tan x$，令$\tan x=t$，可将方程化为常系数。
...
> [!note] 分析
> 该方程不是线性微分方程，也不属于可降阶方程类型。但方程中含 x 的项仅有一次形式，可考虑将 y 作为自变量，x 作为未知函数来转化方程形式。

> [!answer]- 解答
> 因为$\frac{dy}{dx}=\left(\frac{dx}{dy}\right)^{-1}$，则
>
...
**例 1**：在13时到14时的什么时刻，一个时钟的分针恰好与时针重和.

> [!note] 分析
> 若 t 时刻分针与时针分别位于$x(t)$和$y(t)$处，则由$x(t)=y(t)$就可求得两针重和的时间，所以问题的关键是求得$x(t)$与$y(t)$的表达式。由于分针与时针都是做匀速运动，其位置函数是容易求得的。

> [!answer]- 解答
> 将圆周角 60 等分，设每份为一个单位，又设$t \, (\text{min})$时刻分针与时针分别位于$x(t)$和$y(t)$处，由于初始时间为 13 时，分针与时针的速度分别为 1 （单位/min）与 5/60 （单位/min），故有
>
...
**例 2**：设$f(x)$是区间$[0, +\infty)$上具有连续导数的单调递增函数，且$f(0)=1$，对任意的$t\in[0, +\infty)$，

直线 x=0, x=t，曲线$y=f(x)$以及 x 轴所围成的曲边梯形绕 x 轴旋转一周生成一旋转体，若该旋转体的侧面积在数值上等于其体积的 2 倍，求$f(x)$的表达式.

> [!note] 分析
> 只需由$f(x)$分别表示其旋转体的体积与侧面积就可写出方程.

> [!answer]- 解答
> 旋转体的体积$V=\pi\int_{0}^{t}f^{2}(x)dx$，侧面积$S=2\pi\int_{0}^{t}f(x)\sqrt{1+[f'(x)]^{2}}dx$.
>
> 由题设条件知
>
...
**例 3**：求一曲线，使其上每一点的向径与切线的夹角等于切线斜角的$\frac{1}{3}$

> [!note] 分析
> 要刻画曲线上点的向径与角度，用极坐标表示更为方便。利用已知条件及导数的几何意义就可建立微分方程。

> [!answer]- 解答
> 设曲线的极坐标方程为$\rho=\rho(\theta)$，$\varphi$表示曲线上点$(\rho,\theta)$处极径与曲线切线的夹角，切线的斜角为$\alpha$（如图 7.1 所示），则
>
...
**例 4**：设函数$y = f(x)$在$\left[\frac{1}{2}, +\infty\right)$上连续，且$f(2) = \frac{4}{3}$. 若曲线$y = f(x)$与直线$x = \frac{1}{2}$，$x = t$（$t > \frac{1}{2}$）及 x 轴围成的平面图形$D_t$绕 x 轴旋转一周而成的旋转体体积为$V(t) = \frac{\pi}{2} \left[ 4t^2 f(t) - f\left( \frac{1}{2} \right) \right]$，求$D_t$绕 y 轴旋转一周而成的旋转体体积$V_y$.

> [!note] 分析
> 利用旋转体体积公式$V(t)=\pi\int_{\frac{1}{2}}f^{2}(x)dx$与题设条件可建立未知函数$f(x)$的积分方程，求导可得到微分方程. 解出$f(x)$后再求$V_{y}$.
>
...
> [!note] 分析
> 要计算 PQ 的长度，需知道点 Q 的坐标，从而需先写出曲线在点 P 处的法线方程.

> [!answer]- 解答
> 设曲线方程为$y=f(x)$，则它在点 P 处的法线方程为$Y-y=-\frac{1}{y^{\prime}}(X-x)$.
>
...
**例 6**：一小船A从原点出发，以匀速$v_{0}$沿y轴正向行驶。另一小船B从x轴上的点$(x_{0},0)(x_{0}<0)$出发，朝A追去，其速度方向始终指向A，速度大小为常数$v_{1}$.
...
> [!note] 分析
> 这是追击问题. 它所隐含的等量关系有两个：一是船 B 的运动方向（切线方向）始终为 BA 方向；二是两船在相同时间段所走过的路程比等于其速度比. 若船 B 的运动轨迹为$y = y(x)$，则船 B 能追上船 A 的标志是$y(x)$在 x = 0 有意义或$\lim_{x \to 0} y(x)$存在.

> [!answer]- 解答
> （1）设船 B 的轨迹线为$y = y(x)$。经过时间 t，两船的位置分别为$A(0, v_{0}t)$与$B(x, y)$。因为船 B 的速度的方向指向船 A（如图 7.2 所示），故
>
...
**例 7**：如图7.3所示，设河宽为a，一条船从岸边一点O出发驶向对岸，船头总是指向对岸与点O相对的一点B. 假设船在静水中的船速为常数$v_{1}$，河流中水的流速为常数$v_{2}$，试求船过河所走的路线（曲线方程），并讨论在什么条件下船能到达对岸，以及船能到达点B.
...
> [!answer]- 解答
> 如图建立坐标系，则点$B$的坐标为$(0,a)$，设在时刻$t$船的位置为$P(x,y)$，船实际速度$\vec{v}$是由船的航行速度$v_1 = \left(-\frac{v_1x}{\sqrt{x^2+(a-y)^2}}\right)$，$\frac{v_1(a-y)}{\sqrt{x^2+(a-y)^2}}$与水流速度$v_2 = (v_2,0)$的合成$\vec{v} = v_1 + \vec{v}_2$，所以有
>
...
> [!note] 分析
> 昆虫的实际爬行速度是圆盘的转动速度与昆虫的爬行速度的合成，由此可建立昆虫运动的微分方程.

> [!answer]- 解答
> 取圆盘的中心为原点，如图7.4所示建立坐标系. 昆虫开始在点$\left(\frac{3}{2},0\right)$处，光源在$(-∞,0)$处. 圆盘按反时钟方向旋转. 假设在时间t昆虫位于$(x,y)$，即$(r,\theta)$处，此时昆虫随圆盘转动的方向为（圆周切线方向）$e_{\tau}=\left(\frac{-y}{r},\frac{x}{r}\right)$，昆虫的爬行方向为$e_{s}=(-1,0)$（当光源离圆盘较远时，可视$e_{s}$不变），则昆虫的实际运动速度为$\omega r e_{\tau}+e_{s}\left(\omega=\frac{2\pi}{15}\right)$，即有$\frac{dx}{dy}=ey-1$ ${}^{①}$ $\frac{dy}{dx}=ex$ ${}^{②}$
>
...
> [!note] 分析
> 桥墩侧面是旋转曲面，只需求得一条母线即可. 当每个截面圆上的压强等于允许压强时，是材料最省的情况. 可由此建立方程.

> [!answer]- 解答
> 如图7.5所示，设桥墩侧面由曲线$y = f(x)$绕x轴旋转而成. 在 x 点处截面上所受的总载荷为上方的压力 P 与区间$[0, x]$上桥墩的自重之和，即$P + \mu g \pi \int_{0}^{x} y^{2}(t) \, dt$. 而该截面的面积为$\pi y^{2}(x)$，它允许承受的压强为$k \, kN/m^{2}$，得
>
...
> [!note] 分析
> 应先求得甲容器中含盐量的变化规律，为此可考虑经过较短时间段$[t,t+\Delta t]$，甲容器中含盐量的变化（等量关系：含盐改变量 = 流入量 - 流出量)，取极限建立微分方程，再求解. 类似可求得乙容器中含盐量的变化规律.

> [!answer]- 解答
> 设$t$时刻，甲容器中含盐量为$x(t)$，则其浓度为$\frac{\sigma}{100}$，由$t$到$t+\Delta t$时间段甲容器中盐的改变量为$\Delta x \approx -\frac{x}{100}2\Delta t$，两边除以$\Delta t$，取极限$\Delta t \to 0$得
>
...
> [!note] 分析
> 需先求得飞机滑行的运动方程. 由于飞机受到的外力仅有空气阻力, 根据牛顿第二定律很容易建立微分方程.

> [!answer]- 解答
> 方法1 由题设，飞机的质量 m = 9000kg，着陆时的水平速度$v_{0} = 700km/h$。从飞机接触跑道时开始计时，设 t 时刻飞机的滑行距离为 x(t)，速度为 v(t)。根据牛顿第二定律，得
>
>  $$m\frac{\mathrm{d}\nu}{\mathrm{d}t}=-k\nu\;.$$
>
...
> [!note] 分析
> 需先求得链条下滑的运动方程. 链条下滑是由钉子两侧链条的重量差引起的，由牛顿第二定律容易建立微分方程.
>
...
> [!answer]- 解答
> 设链条的线密度为$\mu$，经过时间 t，链条下滑了 x 米，则由牛顿第二定律得
>
